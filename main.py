...
from report import Debrief
from cockpit_hud import Hud
from mission_menu import MissionSelect
from mission_loader import parseMissionFile
...
class Player:
    def __init__(self, worldInstance):
        # Pivot node para lógica de jogo (posição, heading, movimento)
        self.model = render.attachNewNode("player")
        self.model.setZ(19)

        # Mesh visual com correção de orientação: no GLB a nose aponta +Z, precisa apontar +Y
        mesh = loader.loadModel(models["player"])
        mesh.reparentTo(self.model)
        mesh.setScale(0.015)
        mesh.setHpr(125,0, 50)
